...
# Para auxiliar o secretário de Saúde, faça uma função chamada 
# "calcula_media_movel", que deve receber como parâmetros um array 
# com a série de registros de mortes diária por covid e o número de
#  dias de intervalo para calcular as médias. Essa função deve retornar 
#  um novo array sendo que o valor de cada elemento é a média de mortes
#   com base no número de dias de intervalo informado no segundo parâmetro.
# OBS.: O último elemento do novo array deve ser a média dos últimos números da
# série de registros que sobrarem.
# Por exemplo:

def calcula_media_movel (mortes_diaria, dias)
    novo_array = []
    if mortes_diaria.size % dias == 0
        calculo_media = mortes_diaria.size / dias
        resto = 0
    else
        calculo_media = (mortes_diaria.size / dias) + 1
        resto = mortes_diaria.size % dias
    end
  
        for i in (0..calculo_media-1)
            if i == calculo_media - 1 && resto != 0 
                calculo_inicial = dias  * i
                calculo_final = mortes_diaria.size  - 1

                soma = 0.0
                for n in (calculo_inicial..calculo_final)
